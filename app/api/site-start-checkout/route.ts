// Plata Site Start (1.500 lei — site de prezentare 4 pagini) — consultantul trimite clientul aici.
...
import { createSiteStartCheckoutSession, stripeEnabled, siteStartPriceRon } from "@/lib/stripe";
...
export async function POST(req: Request) {
  if (!rateLimit(`site-start-checkout:${getClientIp(req)}`, 10, 60_000)) {
  ...
  if (stripeEnabled()) {
    try {
      const { url } = await createSiteStartCheckoutSession({ origin, email });
      return NextResponse.json({ url });
    } catch (e: any) {
      console.error("[site-start-checkout] stripe failed:", e);
      return NextResponse.json(
  ...
  if (hasDb()) {
    try {
      await insertBrief({
        name: email,
        email,
        selected_package: "Site Start 1500 lei",
        industry: "",
        message: `Cerere Site Start (fără plată online — Stripe neconfigurat). De facturat manual ${siteStartPriceRon()} lei.`,
        source: "site-start-cerere",
      });
      recorded = true;
    } catch (e) {
      console.error("[site-start-checkout] lead insert failed:", e);
    }
  }
  try {
    await sendSimpleEmail({
      to: ownerEmail(),
      subject: `🏗️ CERERE Site Start — ${email}`,
      html: `<p><b>${email}</b> vrea Site Start (${siteStartPriceRon()} lei). Stripe nu e configurat — facturezi manual. Detaliile comenzii sunt în lead-ul trimis de consultant.</p>`,
      replyTo: email,
    });
    recorded = true;
  } catch (e) {
    console.error("[site-start-checkout] owner email failed:", e);
  }
  if (!recorded) {
    return NextResponse.json(
  ...
  insertNotification(
    email,
    "order",
    "🏗️ Comanda ta Site Start e înregistrată",
